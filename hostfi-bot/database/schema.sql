...
-- Admin-reviewed proof that an invited user followed HostFi on X
CREATE TABLE IF NOT EXISTS x_follow_submissions (
    id                      BIGSERIAL PRIMARY KEY,
    cycle_id                 BIGINT NOT NULL REFERENCES campaign_cycles(id),
    invite_join_id           BIGINT NOT NULL REFERENCES campaign_invite_joins(id),
    referrer_telegram_id     BIGINT NOT NULL,
    referee_telegram_id      BIGINT NOT NULL,
    x_account_id             BIGINT REFERENCES x_accounts(id),
...
CREATE INDEX IF NOT EXISTS idx_x_follow_submissions_status
    ON x_follow_submissions (status, created_at);
CREATE INDEX IF NOT EXISTS idx_x_follow_submissions_referrer
    ON x_follow_submissions (cycle_id, referrer_telegram_id);
CREATE INDEX IF NOT EXISTS idx_x_follow_submissions_referee
    ON x_follow_submissions (referee_telegram_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_x_follow_submissions_active_x_account
    ON x_follow_submissions (x_account_id)
