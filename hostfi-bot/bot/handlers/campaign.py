...
async def award_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if args and args[0].lower() != "helpful":
        await update.effective_message.reply_text(
            "Usage: reply to a helpful message with <code>/award</code>",
            parse_mode="HTML",
        )
        return
...
async def xpost_review_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "approve":
        ...
        if not event:
            await query.answer("Could not award XP for this submission.", show_alert=True)
            return
        ...
        await query.answer(f"Approved for {amount} XP.")
        try:
            await context.bot.send_message(
